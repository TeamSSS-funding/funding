...
public class PaymentController {
    ...
    private String cardRegister(@AuthenticationPrincipal UserDetails userDetails, @ModelAttribute Card cardProcess) {
        cardProcess.setUserId(userDetails.getId());
        log.debug(userDetails.getId().toString());
        paymentService.createCard(cardProcess);
        return "newCard";
    }

    @GetMapping(value = "profile/cards")
    private String getCardList(@AuthenticationPrincipal UserDetails userDetails, Model model) {
//        cardinfo.setUserId(userDetails.getUsername());
//        List<Cardinfo> cardList = paymentService.cardList(userDetails.getUsername());
